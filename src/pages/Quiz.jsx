...
import { setScore, resetScore } from "../redux/quizSlice";
const Quiz = () => {
  const { id } = useParams();
  ...
  // Prevent Reattempt
  useEffect(() => {
    if (attempts[`${user.name}_${quiz?.id}`]) {
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      ...
      e.returnValue = "Quiz is in progress";
    };
  ...
  const [current, setCurrent] = useState(
    Number(localStorage.getItem(`currentQuestion_${id}`)) || 0,
  );
  ...
  // Save Timer
  useEffect(() => {
    localStorage.setItem(`timer_${id}`, time);
  }, [time]);

  // Save Current Question
  useEffect(() => {
    localStorage.setItem(`currentQuestion_${id}`, current);
  }, [current]);
  ...
  const finishQuiz = (finalScore = score) => {
    ...
    // Lock Attempt
    attempts[`${user.name}_${quiz.id}`] = true;
    ...
    localStorage.removeItem(`currentQuestion_${id}`);
  ...
  const handleAnswer = (option) => {
    ...
    const updatedScore =
      option === quiz.questions[current].answer ? score + 1 : score;
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
            ...
            <div>
              ...
              <p className="text-red-300 mt-3 text-sm text-center">
                Quiz auto submits when timer ends
              </p>
            </div>
          </div>
        ...
        <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl shadow-2xl p-10 transition-all duration-500">
          ...
          <div className="mt-8 bg-red-500/10 border border-red-400 text-red-300 p-5 rounded-2xl text-lg">
            🔒 Once answered, you cannot return to previous questions
          </div>
        </div>
      </div>
    </div>
